...
public class ClassFileWriter {
	...
	private void translate(Stmt.Break stmt, Context context, List<Bytecode> bytecodes) {
		bytecodes.add(Goto(endOf(nests.peek())));
	}
	...
	private void translate(Expr.ArrayInitialiser expr, Context context, List<Bytecode> bytecodes) {
		...
		for (Expr arg : expr.getArguments()){
			translate(arg, context, bytecodes);
			Attribute.Type attr = arg.attribute(Attribute.Type.class);
			...
			bytecodes.add(new Bytecode.Invoke(JAVA_UTIL_ARRAYLIST, "add", ftype, InvokeMode.VIRTUAL));
		}
	...
	private void translate(Expr.Binary expr, Context context, List<Bytecode> bytecodes) {
		...
		case AND:
			...
			break;
		case OR:
			bytecodes.add(new Bytecode.BinOp(Bytecode.BinOp.OR, lhsType));
			break;
		case ADD:
			bytecodes.add(new Bytecode.BinOp(Bytecode.BinOp.ADD, lhsType));
			break;
		case SUB:
			bytecodes.add(new Bytecode.BinOp(Bytecode.BinOp.SUB, lhsType));
			break;
		case DIV:
			bytecodes.add(new Bytecode.BinOp(Bytecode.BinOp.DIV, lhsType));
			break;
		case MUL:
			bytecodes.add(new Bytecode.BinOp(Bytecode.BinOp.MUL, lhsType));
			break;
		case REM:
			bytecodes.add(new Bytecode.BinOp(Bytecode.BinOp.REM, lhsType));
			break;
		case EQ:
			simpleCompare(Bytecode.IfCmp.EQ, lhsType, bytecodes);
			break;
		case NEQ:
			simpleCompare(Bytecode.IfCmp.NE, lhsType, bytecodes);
			break;
		case LT:
			simpleCompare(Bytecode.IfCmp.LT, lhsType, bytecodes);
			break;
		case LTEQ:
			simpleCompare(Bytecode.IfCmp.LE, lhsType, bytecodes);
			break;
		case GT:
			simpleCompare(Bytecode.IfCmp.GT, lhsType, bytecodes);
			break;
		case GTEQ:
			simpleCompare(Bytecode.IfCmp.GE, lhsType, bytecodes);
			break;
		default:
			throw new IllegalArgumentException("unknown binary expression encountered: " + expr);
		}
	}

	private void simpleCompare(int comparison, JvmType type, List<Bytecode> bytecodes) {
	...
	private void translate(Expr.Unary expr, Context context, List<Bytecode> bytecodes) {
		...
		case NEG:
			...
			break;
		default:
			System.out.println("unknown unary operator encountered: "+ expr.getOp());
			throw new IllegalArgumentException("unknown unary operator encountered: "+ expr.getOp());
		}
	}
	...
	/**
	 * Get a new loop label name which has not been used before.
	 *
	 * @return
	 */
	...
	/**
	 * Get a new switch label name which has not been used before.
	 *
	 * @return
	 */
	...
	/**
	 * Get a new invoke label name which has not been used before.
	 *
	 * @return
	 */
	...
	/**
	 * Get a new case label name which has not been used before.
	 *
	 * @return
	 */
	...
	private String startOf(String loop) {
		return "start" + loop;
	}

	private String bodyOf(String loop) {
		return "body" + loop;
	}

	private String endOf(String loop) {
		return "end" + loop;
	}

	private String conditionOf(String loop) {
		return "cond" + loop;
	}

	private Bytecode Goto(String label){
		return new Bytecode.Goto(label);
	}

	private Bytecode label(String label){
		return new Bytecode.Label(label);
	}
